...
contexts:
  ...
  remote-cluster:
    ...
    sasl:
      ...
      # alternatively, use a script to retrieve a token dynamically (see plugin section):
      # tokenprovider:
      #   plugin: generic
      #   options:
      #     # path to an executable script that outputs a JSON with format `{"token": "<token>", "extensions": {"<key>": "<value>"}` where `extensions` is optional
      #     script: /path/to/token-script.sh
